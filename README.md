...
其中CuteCloud 联盟用户指 用户使用的COS可以减少一定费用 使用自定义域名需要使用国内外第三方CDN，不支持Cloudflare和腾讯云CDN、TEO 因为底层是腾讯云COS 只要支付储存费用基本上随便用。我这里大概是不到100/tb吧 其中 自带域名 免流量费 只需要支付储存费用 CDN费用无需支付，月流量限制20GB， 不管是否使用单用户限速100MB/S但是这个项目是不对外的 管理员只会给认识的人，后续等搞了ICP EDI再说吧 (
