...
第一 jsd.cdn.zzko.cn 还会保持正常运营的 但是 可能不会在说是对于底层系统的调整和变动 不会在出现小幅度的更新了 只有大更新或者说有关部门的要求什么的才会更新，当然审计系统依然会正常工作的

第二 产品侧计划 使用新的独立域名 内部版本代号 test5bate 新版也就是独立域名比如说你的域名是 mwm.moe 那么你的加速就是 mwm-moe.cdn.testv5cdn.cn 或者说是lizi.cdn.testv5cdn.cn 当然这个域名不一定是这个域名，相关页面还在开发中 暂时不支持透露 新系统拥有独立的白名单和独立的地址进行加速 避免大锅饭 一人违规全体受罚，但是我们内部纪录的黑名单现阶段将无法申请了哈 这个独立域名现阶段可能有要求 比如说是友情链接或者说底部悬挂还是什么 内部目前还在讨论中 等上线了我在来更新文档
...
后期会更换域名的 嗯，不建议直接用这个域名去重定向回源，出了问题一律不管(

审核系统由于没有模型使用的()腾讯云的平台常见模型可能出现误封
