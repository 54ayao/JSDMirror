...
2.此外监测点反馈重庆地域解析异常 返回IP为 127.0.0.0.1  0.0.0.0 

监控到域名已经被列入反诈，如果你的地区无法访问的话，请联系我，我们和反诈中心和运营商进行沟通。

已知 重庆96110在该市下发指令封堵。，已知重庆移动，电信 (正在和当地市96110反诈中心沟通中。)必要的时候我们将提供日志。给政府机构。

3.鉴于此次平台将再次调整一下审核模型，后续对于封禁的路径将不进行公开 仅作为内部存档使用

4.平台将重新审核3月期间的审核系统的审核结果 将审核结果同步第三方人员审核在此确认，优化用户体验，升级审核算法 执行事不过三策略 当月 违规不超过3次切不满足违规每次3张 无故意因素  将不处罚 只屏蔽图片 （4月20号实行）
...
2.可以申请不审核 但是仅限于ACG、正常图片、无明显露点、博客标记、日常生活 等等 需要有一定的信誉累计
