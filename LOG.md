...
所以将在2023年1月1日暂停腾讯CDN的接入。（PS：考虑了一下还是选择腾讯）
#### 2022年11月2日
...
2023年起可能只保留金山、百度、网宿、白山CDN。 （PS：考虑了一下还是选择腾讯）

因为腾讯云增加HTTPS计费，将会对其暂停使用，
...
推荐使用朋友的 GitHub 镜像站：https://kgithub.com
...
#### 2023年8月3日
后台监控到cdn大量cloudflare 502 并且速度出奇的慢
经过排查 由于 cdn.jsdelivr.net 多个运营商再一次拉入黑名单。进行sni阻断
后尝试切换备用线路 cdn.jsdelivr.us  然后才发生的502错误。刷一下又好，按理来说不应该这么慢 应该是带了sni和回源域名不相符问题。
后续改成发送域名为fastly.jsdelivr.net，后台监控恢复正常速度成两秒提升到0.35秒，目前依旧使用于函数之类的产品运营。
